...
const AdminStats: React.FC<{ products: Product[], orders: Order[] }> = ({ products, orders }) => {
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      <h1 className="text-4xl md:text-5xl font-black tracking-tight">Analytics <span className="text-slate-600">Core</span></h1>
...
const AdminProducts: React.FC<{ products: Product[], setProducts: (p: Product[]) => void }> = ({ products, setProducts }) => {
  ...
  const initialFormState: Partial<Product> = {
    ...
    isFree: false, badgeText: '', fileType: 'LINK', fileSize: '0 MB', imageUrl: '',
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      const productPayload = {
        ...
        badge_text: formData.badgeText, rating: 5.0, sales_count: 0
      };
  ...
  return (
    <div className="space-y-10">
      <div className="flex justify-between items-end">
        ...
        <button onClick={openAddModal} className="px-8 py-4 bg-indigo-600 text-white rounded-[24px] font-black flex items-center gap-3">
          ...
          <tbody className="divide-y divide-slate-800/50">
            {products.map(p => (
              <tr key={p.id} className="hover:bg-slate-800/20">
                <td className="px-10 py-6"><div className="flex items-center gap-4"><img src={p.imageUrl} className="w-10 h-10 rounded-lg object-cover" /><p className="font-black">{p.title}</p></div></td>
                <td className="px-6 py-6 font-black text-indigo-400">{p.isFree ? 'FREE' : `$${p.price}`}</td>
                <td className="px-10 py-6 text-right space-x-3">
                  <button onClick={() => openEditModal(p)} className="p-2 text-indigo-400"><Edit3 size={16} /></button>
                  <button className="p-2 text-rose-500"><Trash2 size={16} /></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/98 backdrop-blur-xl">
          <div className="bg-slate-900 border border-white/5 w-full max-w-4xl rounded-[48px] shadow-2xl overflow-hidden p-10 space-y-8 animate-in zoom-in duration-300">
            <div className="flex justify-between items-center">
              <h2 className="text-3xl font-black">{editingId ? 'Modify' : 'Create'} Asset</h2>
              <button onClick={() => setIsModalOpen(false)} className="p-3 bg-slate-800 rounded-2xl"><X /></button>
            </div>

            <form onSubmit={handleSaveProduct} className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                <div onClick={() => imageInputRef.current?.click()} className="aspect-video bg-slate-950 rounded-[32px] border-4 border-dashed border-slate-800 flex items-center justify-center cursor-pointer overflow-hidden">
                  {previewImg ? <img src={previewImg} className="w-full h-full object-cover" /> : <ImageIcon size={32} className="text-slate-800" />}
                ...
                <input placeholder="Product Title" className="w-full px-6 py-4 bg-slate-800 rounded-2xl font-bold" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
                <textarea placeholder="Description" rows={3} className="w-full px-6 py-4 bg-slate-800 rounded-2xl font-bold" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
              </div>

              <div className="space-y-6">
                <div className="p-6 bg-slate-950 rounded-[32px] space-y-4">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest flex items-center gap-2"><Tag size={12}/> Access Type</label>
                    <button type="button" onClick={() => setFormData({...formData, isFree: !formData.isFree, price: !formData.isFree ? 0 : formData.price})} className={`px-4 py-1 rounded-full text-[10px] font-black uppercase ${formData.isFree ? 'bg-emerald-500' : 'bg-slate-700'}`}>{formData.isFree ? 'Free' : 'Paid'}</button>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <input placeholder="Badge (NEW)" className="w-full px-4 py-3 bg-slate-800 rounded-xl text-xs font-bold" value={formData.badgeText} onChange={e => setFormData({...formData, badgeText: e.target.value})} />
                    <input type="number" step="0.01" disabled={formData.isFree} placeholder="Price" className="w-full px-4 py-3 bg-slate-800 rounded-xl text-xs font-bold" value={formData.price} onChange={e => setFormData({...formData, price: parseFloat(e.target.value) || 0})} />
                  </div>
                </div>

                <select className="w-full px-6 py-4 bg-slate-800 rounded-2xl font-bold" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value as Category})}>
                  {(Object.values(Category) as string[]).map(cat => <option key={cat} value={cat}>{cat}</option>)}
                </select>

                <input placeholder="Download URL" className="w-full px-6 py-4 bg-slate-800 rounded-2xl font-bold" value={formData.fileUrl} onChange={e => setFormData({...formData, fileUrl: e.target.value})} />

                <button type="submit" disabled={isSaving} className="w-full py-5 bg-indigo-600 text-white font-black text-xl rounded-3xl">
                  {isSaving ? <Loader2 className="animate-spin mx-auto" /> : 'Deploy Asset'}
                </button>
              </div>
            </form>
          </div>
...
const AdminRequests: React.FC = () => {
  ...
  return (
    <div className="space-y-12">
      <h1 className="text-4xl font-black text-white">Inquiries</h1>
      <div className="bg-slate-900 border border-slate-800 rounded-[48px] overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-800/50 text-[10px] font-black text-slate-500 uppercase tracking-widest">
            <tr><th className="px-10 py-6">Request</th><th className="px-6 py-6">Votes</th><th className="px-10 py-6 text-right">Status</th></tr>
          </thead>
          <tbody className="divide-y divide-slate-800/50">
            {requests.map(r => (
              <tr key={r.id} className="hover:bg-slate-800/20"><td className="px-10 py-6 font-black">{r.title}</td><td className="px-6 py-6 font-black">{r.votes}</td><td className="px-10 py-6 text-right uppercase text-[10px] font-black">{r.status}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
